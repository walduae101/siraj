...
async function migrateUserPoints() {
  ...
  try {
    ...
    for (const doc of userPointsSnapshot.docs) {
      ...
      const paidPoints =
        typeof data.paidPoints === "string"
          ? Number.parseInt(data.paidPoints, 10)
          : data.paidPoints || 0;

      const userId = data.userId || uid;

      if (Number.isNaN(paidPoints) || paidPoints <= 0) {
      ...
      console.log(
        `Current balance: ${currentBalance}, Adding: ${paidPoints}, New balance: ${newBalance}`,
      );
      ...
      await db.runTransaction(async (tx) => {
        ...
        tx.set(
          walletRef,
          {
            ...
            promoBalance: walletDoc.exists
              ? walletDoc.data()?.promoBalance || 0
              : 0,
            promoLots: walletDoc.exists
              ? walletDoc.data()?.promoLots || []
              : [],
            updatedAt: Timestamp.now(),
        ...
        tx.set(ledgerRef, {
          ...
          pre: {
            paid: currentBalance,
            promo: walletDoc.exists ? walletDoc.data()?.promoBalance || 0 : 0,
          },
          post: {
            paid: newBalance,
            promo: walletDoc.exists ? walletDoc.data()?.promoBalance || 0 : 0,
      ...
      console.log(
        `✅ Successfully migrated ${paidPoints} points to user ${userId}`,
      );
    }
