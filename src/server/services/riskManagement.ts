...
export class RiskManagementService {
  ...
  static async checkVelocity(check: VelocityCheck): Promise<VelocityResult> {
    ...
    if (hourlyCredits + check.amount > 200) {
      riskScore += 50;
      riskReasons.push(
        `Hourly credit limit exceeded: ${hourlyCredits + check.amount}/200`,
      );
    }
    ...
    if (dailyCredits + check.amount > 800) {
      riskScore += 30;
      riskReasons.push(
        `Daily credit limit exceeded: ${dailyCredits + check.amount}/800`,
      );
    }
    ...
    if (check.customerId) {
      ...
      if (accountsWithCustomerId > 3) {
        riskScore += 60;
        riskReasons.push(
          `Multiple accounts with same customer ID: ${accountsWithCustomerId}`,
        );
      }
    }
    ...
    if (accountAge < 60 && hourlyCredits + check.amount > 100) {
      riskScore += 40;
      riskReasons.push(
        `New account with high velocity: ${accountAge}min old, ${hourlyCredits + check.amount} credits/h`,
      );
    }
    ...
    if (check.ip) {
      ...
      if (ipCredits + check.amount > 500) {
        riskScore += 35;
        riskReasons.push(
          `IP-based velocity exceeded: ${ipCredits + check.amount}/500`,
        );
      }
    }
  ...
  static async createRiskEvent(
    ...
    MetricsService.recordRiskHoldCreated(
      uid,
  ...
  static async getRiskStatistics(): Promise<{
    ...
    const riskScores = totalHoldsSnapshot.docs.map(
      (doc) => doc.data().riskScore || 0,
    );
    const avgRiskScore =
      riskScores.length > 0
