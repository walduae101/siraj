...
export default function FraudReviewsPage() {
  ...
  const { data: reviews, refetch: refetchReviews } =
    api.fraud.admin.reviews.list.useQuery(
  ...
  return (
    <div className="container mx-auto space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold text-3xl">Manual Reviews</h1>
      ...
      <Tabs defaultValue="queue" className="space-y-4">
        ...
        <TabsContent value="queue" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {filteredReviews.filter((r: any) => r.status === "open").length >
              0 ? (
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="flex flex-col space-y-2">
                            ...
                            <div className="text-muted-foreground text-sm">
                              Score: {review.decision?.score} | Confidence:{" "}
                              {review.decision?.confidence}%
                            </div>
                            <div className="text-muted-foreground text-sm">
                              Product: {review.checkout?.productId} | Qty:{" "}
                              {review.checkout?.quantity} | Price: $
                              {review.checkout?.price}
                            </div>
      ...
      <Dialog open={isReviewDialogOpen} onOpenChange={setIsReviewDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Review the flagged transaction and decide whether to approve or
              deny
            </DialogDescription>
          </DialogHeader>
          {selectedReview && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>User ID</Label>
                  <div className="font-mono text-sm">{selectedReview.uid}</div>
                </div>
                <div>
                  <Label>Status</Label>
                ...
                <div>
                  ...
                  <div className="text-sm">
                    {selectedReview.decision?.score}
                  </div>
                </div>
                <div>
                  <Label>Confidence</Label>
                  <div className="text-sm">
                    {selectedReview.decision?.confidence}%
                  </div>
                </div>
                <div>
                  <Label>Product</Label>
                  <div className="text-sm">
                    {selectedReview.checkout?.productId}
                  </div>
                </div>
                <div>
                  <Label>Quantity</Label>
                  <div className="text-sm">
                    {selectedReview.checkout?.quantity}
                  </div>
                </div>
                <div>
                  <Label>Price</Label>
                  <div className="text-sm">
                    ${selectedReview.checkout?.price}
                  </div>
                </div>
                <div>
                  <Label>Created</Label>
              ...
              <div>
                ...
                <div className="mt-2 space-y-1">
                  {selectedReview.decision?.reasons?.map(
