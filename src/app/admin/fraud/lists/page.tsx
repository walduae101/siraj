...
import { Plus, Trash2, Shield, CheckCircle, XCircle, Clock, User, Globe, CreditCard, Monitor } from "lucide-react";
import { toast } from "sonner";

export default function FraudListsPage() {
  const [selectedList, setSelectedList] = useState<"denylist" | "allowlist">("denylist");
  ...
  // Fetch lists
  const { data: denylist, refetch: refetchDenylist } = api.fraud.admin.lists.list.useQuery(
    { list: "denylist" },
    { refetchInterval: 30000 }
  );

  const { data: allowlist, refetch: refetchAllowlist } = api.fraud.admin.lists.list.useQuery(
    { list: "allowlist" },
    { refetchInterval: 30000 }
  );
  ...
  const addToDenylist = api.fraud.admin.lists.addToDenylist.useMutation({
    onSuccess: () => {
      ...
      setNewEntry({ type: "ip", value: "", reason: "", notes: "", expiresAt: "" });
    },
    onError: (error) => {
  ...
  const removeFromDenylist = api.fraud.admin.lists.removeFromDenylist.useMutation({
    onSuccess: () => {
  ...
  const removeFromAllowlist = api.fraud.admin.lists.removeFromAllowlist.useMutation({
    onSuccess: () => {
  ...
  const cleanupExpired = api.fraud.admin.lists.cleanupExpired.useMutation({
    onSuccess: (result) => {
      toast.success(`Cleaned up ${result.denylist + result.allowlist} expired entries`);
      refetchDenylist();
      refetchAllowlist();
    },
    onError: (error) => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "uid": return <User className="h-4 w-4" />;
      case "emailDomain": return <Globe className="h-4 w-4" />;
      case "device": return <Monitor className="h-4 w-4" />;
      case "bin": return <CreditCard className="h-4 w-4" />;
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "bin": return "Card BIN";
      default: return type;
    }
  };

  const currentList = selectedList === "denylist" ? denylist : allowlist;
  const filteredList = selectedType ? currentList?.filter(entry => entry.type === selectedType) : currentList;

  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Fraud Lists Management</h1>
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Add to {selectedList === "denylist" ? "Denylist" : "Allowlist"}</DialogTitle>
                <DialogDescription>
                  Add a new entry to the {selectedList === "denylist" ? "deny" : "allow"} list
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder={newEntry.type === "ip" ? "192.168.1.1" : "Enter value"}
                  />
                </div>
                <div>
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button onClick={handleAddEntry} disabled={addToDenylist.isPending || addToAllowlist.isPending}>
                    Add Entry
                  </Button>
                </div>
              </div>
      ...
      <Tabs value={selectedList} onValueChange={(value: string) => setSelectedList(value as "denylist" | "allowlist")}>
        <TabsList>
          <TabsTrigger value="denylist" className="flex items-center space-x-2">
            <XCircle className="h-4 w-4" />
            <span>Denylist ({denylist?.length || 0})</span>
          </TabsTrigger>
          <TabsTrigger value="allowlist" className="flex items-center space-x-2">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <span>{selectedList === "denylist" ? "Denylist" : "Allowlist"} Entries</span>
              ...
              {filteredList && filteredList.length > 0 ? (
                <div className="space-y-4">
                  {filteredList.map((entry) => (
                    <div key={`${entry.type}-${entry.value}`} className="flex items-center justify-between p-4 border rounded-lg">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            ...
                            <span className="text-xs text-muted-foreground">
                              Added by {entry.addedBy.substring(0, 8)}... on {entry.addedAt.toLocaleDateString()}
                            </span>
                            {entry.expiresAt && (
                              <span className="text-xs text-muted-foreground">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleRemoveEntry(entry.type, entry.value)}
                        disabled={removeFromDenylist.isPending || removeFromAllowlist.isPending}
                      >
                ...
                <div className="text-center py-8 text-muted-foreground">
                  No {selectedList === "denylist" ? "denylist" : "allowlist"} entries found
                </div>
              )}
            </CardContent>
          </Card>
