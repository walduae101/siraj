...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `[process-order] Processing order ${orderId} for user ${userId}`,
    );
    ...
    if (ledgerDoc.exists) {
      ...
      const alreadyProcessed = existingEntries.some(
        (entry: any) =>
          entry.source?.orderId === orderId ||
          entry.source?.eventId === orderId,
      );

      if (alreadyProcessed) {
        console.log(
          `[process-order] Order ${orderId} already processed for user ${userId}`,
        );
        return NextResponse.json({
          success: true,
          credited: 0,
    ...
    console.log(
      `[process-order] Crediting ${pointsToCredit} points for order ${orderId}`,
    );
    ...
    console.log(
      `[process-order] Successfully credited ${pointsToCredit} points for user ${userId}`,
    );
