...
async function main() {
  ...
  try {
    ...
    for (const [sku, paynowProductId] of Object.entries(
      ...
      console.log(
        `✅ Created product: ${sku} (${paynowProductId}) - ${docRef.id}`,
      );
    }
    ...
    for (const [planKey, planData] of Object.entries(
      ...
      const details =
        subscriptionDetails[planKey as keyof typeof subscriptionDetails];
      if (!details) {
      ...
      if (!existingQuery.empty) {
        console.log(
          `✓ Subscription product already exists: ${planKey} (${paynowProductId})`,
        );
        continue;
      }
      ...
      const docRef = await db.collection("products").add(productData);
      console.log(
        `✅ Created subscription product: ${planKey} (${paynowProductId}) - ${docRef.id}`,
      );
    }
