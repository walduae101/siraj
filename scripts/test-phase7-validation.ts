...
async function testPhase7Validation() {
  ...
  try {
    ...
    console.log(`   multiRegion.enabled: ${config.features.multiRegion.enabled}`);
    console.log(`   multiRegion.primaryRegion: ${config.features.multiRegion.primaryRegion}`);
    console.log(`   multiRegion.secondaryRegion: ${config.features.multiRegion.secondaryRegion}`);
    console.log(`   eventSchema.version: ${config.features.eventSchema.version}`);
    console.log(`   eventSchema.minCompatible: ${config.features.eventSchema.minCompatible}`);
    ...
    console.log(`   Backward compatible: ${currentVersion >= minCompatible ? "✅" : "❌"}`);
    
    // Test version compatibility
    const testVersions = [1, 2, 3, 4];
    for (const version of testVersions) {
      const compatible = version >= minCompatible;
      console.log(`   Version ${version}: ${compatible ? "✅ Compatible" : "❌ Incompatible"}`);
    }

    // Test 3: Region Configuration
    console.log("\n📋 Test 3: Region Configuration");
    console.log(`   Primary region: ${config.features.multiRegion.primaryRegion}`);
    console.log(`   Secondary region: ${config.features.multiRegion.secondaryRegion}`);
    console.log(`   Failover enabled: ${config.features.multiRegion.failoverEnabled ? "✅" : "❌"}`);
    
    // Validate region names
    const validRegions = ["us-central1", "europe-west1", "asia-southeast1"];
    const primaryValid = validRegions.includes(config.features.multiRegion.primaryRegion);
    const secondaryValid = validRegions.includes(config.features.multiRegion.secondaryRegion);
    const regionsDifferent = config.features.multiRegion.primaryRegion !== config.features.multiRegion.secondaryRegion;
    ...
    console.log(`   Multi-region enabled: ${multiRegionEnabled ? "✅" : "⚠️  (disabled)"}`);
    console.log(`   Schema versioning enabled: ${schemaVersioningEnabled ? "✅" : "❌"}`);
    
    if (!multiRegionEnabled) {
      console.log("   ⚠️  Multi-region is disabled - enable for full functionality");
    }
