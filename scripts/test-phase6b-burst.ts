...
class Phase6BBurstTest {
  ...
  private async runPhase(phaseName: string, multiplier: number, duration: number) {
    ...
    console.log(`   📈 Average RPS: ${(messageCount / (duration / 1000)).toFixed(1)}`);
    console.log("");
  }
  ...
  private simulateMetrics(phaseName: string, multiplier: number): BurstTestMetrics {
  ...
  private async generateBurstReport() {
    ...
    console.log("=".repeat(60));
    console.log(`Test Period: ${this.startTime.toISOString()} to ${new Date().toISOString()}`);
    console.log(`Total Metrics Collected: ${this.metrics.length}\n`);

    // Calculate phase averages
    const phases = ["Baseline", "Ramp 1", "Ramp 2", "Ramp 3", "Peak", "Recovery"];
    const phaseResults = phases.map(phase => {
      const phaseMetrics = this.metrics.filter(m => m.timestamp.includes(phase));
      if (phaseMetrics.length === 0) return null;

      const avgWebhookAck = phaseMetrics.reduce((sum, m) => sum + m.webhookAckP95, 0) / phaseMetrics.length;
      const avgWorkerP95 = phaseMetrics.reduce((sum, m) => sum + m.workerP95, 0) / phaseMetrics.length;
      const avgQueueLag = phaseMetrics.reduce((sum, m) => sum + m.queueLag, 0) / phaseMetrics.length;
      const maxDlqDepth = Math.max(...phaseMetrics.map(m => m.dlqDepth));
      const avgErrorRate = phaseMetrics.reduce((sum, m) => sum + m.errorRate, 0) / phaseMetrics.length;
      const loadSheddingCount = phaseMetrics.filter(m => m.loadSheddingActive).length;
    ...
    phaseResults.forEach(result => {
      ...
      console.log(`     Load Shedding: ${result.loadSheddingActive ? 'ACTIVE' : 'inactive'}`);
      console.log("");
    });
    ...
    console.log(`   Recommendations: ${overallAssessment.recommendations.join(', ')}`);
  ...
  private assessBurstResilience(phaseResults: any[]): { status: string; score: number; recommendations: string[] } {
    const recommendations: string[] = [];
    let score = 100;

    // Check webhook ACK performance
    const peakPhase = phaseResults.find(p => p.phase === "Peak");
    if (peakPhase && parseFloat(peakPhase.avgWebhookAck) > 50) {
      score -= 20;
      recommendations.push("Webhook ACK exceeded 50ms at peak load");
    }

    // Check worker performance
    if (peakPhase && parseFloat(peakPhase.avgWorkerP95) > 250) {
      score -= 20;
    ...
    // Check error rate
    if (peakPhase && parseFloat(peakPhase.avgErrorRate) > 1) {
      score -= 15;
