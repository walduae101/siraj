...
async function getFraudMetrics(hours: number = 24): Promise<FraudMetrics> {
  ...
  const processingTimes = decisions.map(d => d.processingMs).sort((a, b) => a - b);
  const avgProcessingMs = processingTimes.reduce((sum, time) => sum + time, 0) / processingTimes.length;
...
async function monitorFraudMetrics() {
  ...
  try {
    ...
    console.log(`   Allow: ${metrics.allowCount} (${((metrics.allowCount / metrics.totalDecisions) * 100).toFixed(1)}%)`);
    console.log(`   Deny: ${metrics.denyCount} (${metrics.denyRate.toFixed(1)}%)`);
    console.log(`   Review: ${metrics.reviewCount} (${((metrics.reviewCount / metrics.totalDecisions) * 100).toFixed(1)}%)`);
    ...
    console.log(`   Average Processing: ${metrics.avgProcessingMs.toFixed(0)}ms`);
    ...
    console.log(`   Deny Rate ≤ 1.0%: ${denyRateReady ? "✅" : "❌"} (${metrics.denyRate.toFixed(1)}%)`);
    console.log(`   Fraud Eval p95 ≤ 150ms: ${processingReady ? "✅" : "❌"} (${metrics.p95ProcessingMs.toFixed(0)}ms)`);
    console.log(`   Webhook p95 ≤ 250ms: ${webhookReady ? "✅" : "❌"} (${metrics.p95ProcessingMs.toFixed(0)}ms)`);
    ...
    if (allReady) {
      ...
      if (!denyRateReady) {
        console.log(`   - Deny rate (${metrics.denyRate.toFixed(1)}%) is above 1.0% target`);
        console.log("   - Consider further tuning of fraud scoring weights");
      }
      if (!processingReady) {
        console.log(`   - Fraud evaluation p95 (${metrics.p95ProcessingMs.toFixed(0)}ms) is above 150ms target`);
    ...
    if (metrics.denyRate > 2.0) {
      console.log("   - Deny rate is high (>2%). Consider increasing thresholds or reducing signal weights.");
    } else if (metrics.denyRate > 1.0) {
      console.log("   - Deny rate is moderate (>1%). Monitor for another 12-24 hours.");
    } else {
      console.log("   - Deny rate is acceptable. Continue monitoring.");
    }

    if (metrics.rateLimitBlocks > 0) {
      console.log(`   - Rate limit blocks detected (${metrics.rateLimitBlocks}). Monitor for patterns.`);
    }
