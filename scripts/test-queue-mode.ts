...
async function testQueueMode() {
  ...
  try {
    ...
    if (success) {
      ...
      console.log(`   ❌ FAILED: WEBHOOK_MODE is '${webhookMode}', expected 'queue'`);
    }
  } catch (error) {
    results.push({
      testName: "Configuration Check",
  ...
  try {
    ...
    const { publishPaynowEvent } = await import("../src/server/services/pubsubPublisher");
    ...
    if (success) {
      console.log(`   ✅ PASSED: Published message ${messageId} in ${duration}ms`);
    } else {
      console.log(`   ❌ FAILED: Publish took ${duration}ms (expected <1000ms)`);
    }
  } catch (error) {
    results.push({
      testName: "Pub/Sub Connectivity",
  ...
  try {
    ...
    if (success) {
      console.log(`   ✅ PASSED: Avg ${Math.round(avgDuration)}ms, p95 ${Math.round(p95Duration)}ms`);
    } else {
      console.log(`   ❌ FAILED: Avg ${Math.round(avgDuration)}ms, p95 ${Math.round(p95Duration)}ms (targets: avg<100ms, p95<200ms)`);
    }
  } catch (error) {
    results.push({
      testName: "Performance Benchmark",
