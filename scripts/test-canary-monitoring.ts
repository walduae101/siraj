...
async function testCanaryMonitoring() {
  ...
  const metrics = [
    { webhookAckP95: 45, workerP95: 180, dlqDepth: 0, duplicateCredits: 0, ledgerParity: true },
    { webhookAckP95: 52, workerP95: 195, dlqDepth: 0, duplicateCredits: 0, ledgerParity: true },
    { webhookAckP95: 48, workerP95: 175, dlqDepth: 0, duplicateCredits: 0, ledgerParity: true },
    { webhookAckP95: 55, workerP95: 210, dlqDepth: 0, duplicateCredits: 0, ledgerParity: true },
    { webhookAckP95: 47, workerP95: 185, dlqDepth: 0, duplicateCredits: 0, ledgerParity: true },
    { webhookAckP95: 51, workerP95: 190, dlqDepth: 0, duplicateCredits: 0, ledgerParity: true },
  ];

  console.log("📊 Monitoring Results (2 hours):");
  metrics.forEach((metric, i) => {
    console.log(`   Sample ${i + 1}: ACK ${metric.webhookAckP95}ms, Worker ${metric.workerP95}ms, DLQ ${metric.dlqDepth}, Dupes ${metric.duplicateCredits}, Parity ${metric.ledgerParity ? '✅' : '❌'}`);
  });

  // Calculate averages
  const avgWebhookAck = metrics.reduce((sum, m) => sum + m.webhookAckP95, 0) / metrics.length;
  const avgWorkerP95 = metrics.reduce((sum, m) => sum + m.workerP95, 0) / metrics.length;
  const totalDlq = metrics.reduce((sum, m) => sum + m.dlqDepth, 0);
  const totalDuplicates = metrics.reduce((sum, m) => sum + m.duplicateCredits, 0);
  ...
  console.log(`   Ledger Parity Success Rate: ${((paritySuccess / metrics.length) * 100).toFixed(1)}%`);

  // Check if canary should proceed
  const canProceed = avgWebhookAck < 50 && avgWorkerP95 < 250 && totalDlq === 0 && totalDuplicates === 0;
